...
import { ModalActivityCategoryData } from "@/types/activityCategory.type";
...
export function CreateCategoryModal({ open, onClose, onSuccess }: CreateCategoryModalProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await ActivityCategoryService.CallCreateCategory(payload);

            if (res?.statusCode === 201 || res?.statusCode === 200) {
                toast.success("Thêm loại hoạt động thành công!");
                onSuccess();
                handleCloseModal();
            } else {
    ...
    return (
        <Dialog open={open} onOpenChange={handleCloseModal}>
            <DialogContent className="max-w-lg w-full p-6">
                <DialogHeader>
                    <DialogTitle className="text-xl font-bold">Thêm loại hoạt động</DialogTitle>
                </DialogHeader>
